...
from SEDR.src.utils_func import mk_dir, adata_preprocess, load_ST_file
import anndata
...
for proj_idx in range(len(proj_list)):
    ...
    np.save(f'result/embedding/SEDR/dlpfc{proj_idx}.npy', sedr_feat)
    # ################## Result plot
    ...
    df_meta = pd.read_csv(f'{data_root}/{data_name}/ground_truth.txt', sep='\t', header=None, index_col=0)
