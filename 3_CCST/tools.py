...
def train(adata, radius, epochs=3000, seed=2022, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), build_graph=True):
    data = utilities.build_graph(adata, radius=radius, lamb=0.8, build_graph=build_graph)
    data_loader = DataLoader([data], batch_size=1)
    adata.obsm['CCST'] = train_DGI(data_loader, len(data.x[0]), 128, epochs, 1e-6, seed, device)
