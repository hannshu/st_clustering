...
class run():
	...
	def _get_cluster_data(
		...
		cluster_adata = anndata.AnnData(adata.obsm["DeepST_embed"])
		cluster_adata.obs_names = adata.obs_names
		sc.pp.neighbors(cluster_adata, n_neighbors = self.n_neighbors)
		if priori:
			res = self._priori_cluster(cluster_adata, n_domains=n_domains)
		else:
			res = self._optimize_cluster(cluster_adata)
		sc.tl.leiden(cluster_adata, key_added="DeepST_domain", resolution=res)
		adata.obs['DeepST_domain'] = cluster_adata.obs['DeepST_domain']
