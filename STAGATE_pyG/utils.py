...
def Transfer_pytorch_Data(adata):
    G_df = adata.uns['Spatial_Net'].copy()  # 每行保存的是(cell1, cell2, distance)
...
def Cal_Spatial_Net(adata, rad_cutoff=None, k_cutoff=None, model='Radius', verbose=True):
    ...
    """
    使用不同的方法来得到最近邻的节点，先用sklearn获得节点以及节点间距离，
    然后分别保存在distances和indices中，最后通过KNN_list传出
    """
    ...
    Spatial_Net = Spatial_Net.loc[Spatial_Net['Distance']>0,]               # 删除所有指向节点本身的记录
